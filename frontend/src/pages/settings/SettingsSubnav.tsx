...
import { SETTINGS_SUBNAV } from './mockSettings';
...
export function SettingsSubnav() {
  ...
  function placePill(instant: boolean) {
    ...
    const active = track.querySelector<HTMLElement>('a.is-settings-tab-active');
    if (!active) {
  ...
  useLayoutEffect(() => {
    ...
    requestAnimationFrame(() => {
      placePill(instant);
      if (!instant) {
        const active = trackRef.current?.querySelector<HTMLElement>('a.is-settings-tab-active');
    ...
    >
      ...
      {SETTINGS_SUBNAV.map((item) =>
        item.ready ? (
          <NavLink
            ...
            className={({ isActive }) =>
              [
                ...
                isActive
                  ? 'is-settings-tab-active text-white'
          ...
          <span
            key={item.to}
            title="Yakında — sırayla açılacak"
