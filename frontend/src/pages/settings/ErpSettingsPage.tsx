...
import { INITIAL_ERP_SETTINGS, type ErpSettings } from './mockErpSettings';

gsap.registerPlugin(useGSAP);

/**
 * Ayarlar › ERP Entegrasyon — Vega bağlantı bilgileri (mock).
 */
export default function ErpSettingsPage() {
  const rootRef = useRef<HTMLDivElement>(null);
  const [draft, setDraft] = useState<ErpSettings>(() => ({ ...INITIAL_ERP_SETTINGS }));
  ...
  function reset() {
    setDraft({ ...INITIAL_ERP_SETTINGS });
    setBaseline({ ...INITIAL_ERP_SETTINGS });
  }
    ...
    <div ref={rootRef} className="w-full">
      <div data-anim className="mb-5">
        ...
        <p className="mt-1 text-sm text-[var(--panel-muted)]">
          Vega erp entegrasyonunu bu sayfadan gerçekleştiriniz.
        </p>
      </div>
