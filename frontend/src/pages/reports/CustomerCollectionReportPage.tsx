...
export default function CustomerCollectionReportPage() {
  ...
  function exportCsv() {
    const header = 'Ünvan;Tahsilat Adedi;Toplam Tutar;Ortalama Tutar\n';
    const body = filtered
      .map(
        (r) =>
          `${r.title};${r.count};${formatMoneyTr(r.total)};${formatMoneyTr(avgOf(r))}`,
      )
      .join('\n');
    const blob = new Blob(['\ufeff' + header + body], { type: 'text/csv;charset=utf-8' });
    const a = document.createElement('a');
    a.href = URL.createObjectURL(blob);
    a.download = 'musteri-tahsilat-raporu.csv';
    a.click();
