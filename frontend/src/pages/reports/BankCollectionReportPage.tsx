...
export default function BankCollectionReportPage() {
  const { token } = useAuth();
  const [filtersOpen, setFiltersOpen] = useState(() => getDefaultFiltersOpen('banka-tahsilat'));
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const data = await api.get<ApiCardCollection>(
        `/api/reports/bank-collection?${qs.toString()}`,
        token,
      );
      setRows(data.rows ?? []);
      ...
      setLoadError(err instanceof Error ? err.message : 'Banka tahsilat raporu yüklenemedi');
      setRows([]);
      setTotalCount(0);
    } finally {
      setLoading(false);
  ...
  function exportCsv() {
    ...
    a.download = 'banka-tahsilat-raporu.csv';
    a.click();
