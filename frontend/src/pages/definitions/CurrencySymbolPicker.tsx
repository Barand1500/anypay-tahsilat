...
import {
  ...
  SYMBOL_GROUP_LABEL,
  type CurrencySymbolOption,
} from './currencySymbols';

type Props = {
  open: boolean;
  value: string;
  onSelect: (option: CurrencySymbolOption) => void;
  onClose: () => void;
};

/**
 * Para birimi sembol seçici — ana modalın sağında açılır.
 */
export function CurrencySymbolPicker({ open, value, onSelect, onClose }: Props) {
  ...
  const filtered = useMemo(() => {
    ...
    if (!q) return CURRENCY_SYMBOL_OPTIONS;
    return CURRENCY_SYMBOL_OPTIONS.filter(
      (o) =>
        o.name.toLocaleLowerCase('tr').includes(q) ||
        o.code.toLocaleLowerCase('tr').includes(q) ||
        o.symbol.toLocaleLowerCase('tr').includes(q),
    );
  }, [query]);

  const grouped = useMemo(() => {
    const map = new Map<CurrencySymbolOption['group'], CurrencySymbolOption[]>();
    for (const o of filtered) {
      const list = map.get(o.group) ?? [];
      list.push(o);
      map.set(o.group, list);
    }
    return (['populer', 'avrupa', 'asya', 'diger'] as const)
  ...
  return (
    <aside
      ...
      className="flex h-[min(72vh,520px)] w-full max-w-2xl shrink-0 flex-col overflow-hidden rounded-2xl border border-[var(--panel-line)] bg-[var(--panel-elevated)] shadow-[0_24px_64px_rgba(0,0,0,0.22)] sm:w-[280px] sm:max-w-none"
    >
      <div className="flex items-start justify-between gap-2 border-b border-[var(--panel-line)] px-4 py-3.5">
        <div>
          ...
          <h3 className="text-base font-bold text-[var(--panel-ink)]">Sembol seç</h3>
        </div>
        <button
          type="button"
          aria-label="Kapat"
        ...
        {grouped.length === 0 ? (
          ...
          grouped.map(({ group, items }) => (
            <div key={group} className="mb-3">
              <p className="px-2 py-1.5 text-[10px] font-bold uppercase tracking-wide text-[var(--panel-muted)]">
                {SYMBOL_GROUP_LABEL[group]}
              </p>
              <ul className="space-y-0.5">
                {items.map((item) => {
                  const active = value === item.symbol;
                  return (
                    <li key={`${item.code}-${item.symbol}`}>
                      <button
                        type="button"
                        data-km-jump
                      ...
                      >
                        <span
                          className={[
                            ...
                            active
                              ...
                              : 'bg-[var(--panel-surface)] text-[var(--panel-ink)] ring-1 ring-[var(--panel-line)]',
                          ].join(' ')}
                        >
                          {item.symbol}
                        </span>
                        <span className="min-w-0 flex-1">
                          ...
                          <span className="block text-[11px] text-[var(--panel-muted)]">
                            {item.code}
                          </span>
                        </span>
                        {active ? (
